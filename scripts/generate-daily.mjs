...
function detectProducts(text = "") {
  const candidates = [
    "断熱材", "塗装剤", "配管", "ユニットバス", "住宅設備", "建具", "収納", "造作材", "階段", "框",
    "接着剤", "添加剤", "梱包資材", "石膏ボード", "ジプスター", "合板", "塩ビ", "防湿シート", "床貼りボンド",
    "TOTO UB", "ノワウッド", "城東テクノ", "ノーリツ特注色", "DAIKEN", "ナフサ"
  ];
...
function sentenceJoin(sentences) {
  return sentences.filter(Boolean).join(" ");
}

function makeAnalysis(article) {
  const text = `${article.title} ${article.summary}`;
  const products = uniq(detectProducts(text));
  const productText = products.length ? products.slice(0, 6).join("、") : "関連する建材・住宅設備";

  if (article.category === "お客様SNS") {
    if (/TOTO|城東テクノ|ノーリツ|受注再開|数量制限|値上げ通告/.test(text)) {
      return sentenceJoin([
        "この投稿は、メーカー側の供給再開情報を顧客が実務目線で再整理している点が重要です。",
        `対象として ${productText} が挙がっており、「再開したが通常納期に戻ったわけではない」「価格確認も必要」という受け止めが出ています。`,
        "きりいーねにとっては、公式発表そのものよりも、購入者が不安に感じる確認項目が見えている投稿です。商品ページで在庫ありと表示するだけでは不十分で、数量制限、納期幅、値上げ予定、代替品の有無まで同じ画面で説明できるかが購買判断に直結します。"
      ]);
    }
    if (/DAIKEN|大量発注/.test(text)) {
      return sentenceJoin([
        "この投稿は、現時点で遅れがないメーカーでも、大量発注には今後対応しにくくなる可能性があるという警戒シグナルです。",
        "通常在庫がある商品でも、工務店やリフォーム会社が複数現場分をまとめて発注した瞬間に供給制約が表面化するリスクがあります。",
        "きりいーねでは単品在庫の有無だけでなく、数量別の出荷可否や分納可否を把握しておかないと、法人顧客の大型注文でトラブルになりやすい局面です。"
      ]);
    }
    if (/建売|新築マンション|引渡し|中古リノベ/.test(text)) {
      return sentenceJoin([
        "この投稿は、建材・設備の納期遅延が資材業界内だけでなく、不動産販売や中古リノベ市場の需給にも波及していることを示しています。",
        "新築の引き渡し延期や建売業者の仕入れ抑制が起きると、短納期で動けるリフォーム・リノベ案件に需要が寄る可能性があります。",
    ...
    return sentenceJoin([
      ...
      "きりいーねでは、この種の投稿をFAQ、商品ページ内の注意文、検索キーワード、メール件名に転用すると、顧客が実際に抱えている不安に近い言葉で接客できます。"
    ]);
  }

  if (/一人親方|小規模事業者|プッシュ型|情報収集|情報発信/.test(text)) {
    return sentenceJoin([
      "国交省が一人親方や小規模事業者に向けてプッシュ型の情報収集・発信を強化する点が重要です。",
      "資材不足の情報は大手団体やメーカーには届いても、個人事業主には遅れて伝わり、直前の発注・現場停止・顧客説明不足につながりやすい構造があります。",
      "きりいーねの主要顧客に小規模工務店や職人が含まれるなら、行政発表を待つのではなく、品目別の供給注意、早期発注、代替候補を短く届ける通知機能が差別化になります。"
    ]);
  }

  if (/ノダ|建具|収納|造作材|階段|框|仕切り価格|7月1日/.test(text)) {
    return sentenceJoin([
      "ノダの価格改定は、床材だけでなく建具・収納・造作材・階段・框まで対象が広がっている点が大きいです。",
      "これらは内装仕上げや引き渡し前工程に関わるため、価格だけでなく見積有効期限、発注タイミング、施主への追加説明に影響します。",
      "主要原材料だけでなく接着剤、添加剤、梱包資材など副資材コストが理由に含まれており、今後ほかの木質建材メーカーにも同様の改定が連鎖する可能性があります。きりいーねでは、対象カテゴリをまとめ買い・値上げ前発注の訴求に変換できるニュースです。"
    ]);
  }

  if (/チヨダウーテ|ジプスター|石膏ボード|不燃仕様|仕様統合/.test(text)) {
    return sentenceJoin([
      "チヨダウーテのジプスター仕様統合は、単なる新商品情報ではなく、原材料調達が不安定な中でメーカーが生産品目を絞り、供給安定を優先する動きです。",
      "準不燃と不燃を統合することで、現場側では従来品番・仕様指定・見積条件・施工説明の確認が必要になります。",
      "石膏ボードは代替が効きそうに見えて、認定、不燃性能、厚み、納まりが絡むため、間違った置き換えは施工トラブルになります。きりいーねでは品番統合情報を商品マスタ、後継品表示、施工注意に正確に反映する必要があります。"
    ]);
  }

  if (/不動産投資|完了検査|引渡し|リフォームオーナー/.test(text)) {
    return sentenceJoin([
      "この内容は、建材・住宅設備の遅延が施工会社だけでなく、不動産投資家やリフォームオーナーの引き渡し・賃貸開始時期に影響することを示しています。",
      "国交省の完了検査の柔軟運用が話題になるほど、設備未納や部分未完了が現実の論点になっています。",
      "きりいーねでは、工務店だけでなくリフォーム事業者が短納期で確保したい設備まわりの副資材、代替可能な補修材、入居前に必要な内装材を優先的に見せることで、遅延リスクを抱えた顧客の受け皿になれます。"
    ]);
  }

  if (/都営住宅|緊急要望/.test(text)) {
    return sentenceJoin([
      "東京都が国交省に緊急要望したという点は、資材不足や価格高騰が個別企業の問題ではなく、公営住宅工事の遂行リスクとして行政課題化していることを示しています。",
      `記事では ${productText} が対象として挙がっており、断熱・配管・浴室など住宅工事の工程を止めやすい品目に焦点が当たっています。`,
      "公的発注側が需給調査、安定供給対策、コスト増支援、価格動向の情報提供を求めているため、民間工務店でも同じ不安が出る可能性が高いです。きりいーねでは、単なる値上げ情報ではなく、どの工程が止まりやすいか、どの代替品を先に提示できるかを整理する必要があります。"
    ]);
  }

  if (/ナフサショック|原材料危機|材料そのものが不安定/.test(text)) {
    return sentenceJoin([
      "この内容は、ナフサショックを単なる値上げではなく、材料入手そのものが不安定になる問題として整理している点が重要です。",
      "価格改定だけなら見積改定で対応できますが、材料が入らない場合は受注停止、納期未定、代替素材の検討まで必要になります。",
      "建材ECでは、価格表の更新だけでは足りません。ナフサ由来の塩ビ、接着剤、断熱材、包装材など、見えにくい副資材まで影響範囲を広げて監視しないと、顧客から見れば突然欠品したように見えて信頼を落とします。"
    ]);
  }

  return sentenceJoin([
    "この内容は建材市場に関わる変化を示していますが、単独で読むよりも、対象品目、発生時期、価格・納期・仕様への影響を分けて見る必要があります。",
    `本文から読み取れる対象は ${productText} で、きりいーねでは商品マスタ、在庫表示、代替提案、FAQのどこに反映すべきかを確認する価値があります。`,
    "特にニュース本文に出ているメーカー名・品目名をそのまま社内確認リストに落とし、仕入れ先確認と商品ページ改善につなげるべきです。"
  ]);
}
...
function makeAction(article) {
  const text = `${article.title} ${article.summary}`;
  const products = uniq(detectProducts(text));
  const productText = products.length ? products.slice(0, 7).join("、") : "該当カテゴリ";

  if (article.category === "お客様SNS") {
    if (/TOTO|城東テクノ|ノーリツ|受注再開|数量制限|値上げ通告/.test(text)) {
      return sentenceJoin([
        `1. ${productText} を対象に、受注再開、数量制限、納期幅、値上げ予定の4項目を商品別に確認する。`,
        "2. 商品ページには「再開済み」だけでなく、「通常納期ではない可能性」「数量制限あり」「価格改定確認中」を表示できる欄を追加する。",
        "3. メルマガまたはトップのお知らせで、再開品目と注意点を一覧化し、代替品・関連副資材も同時に案内する。"
      ]);
    }
    if (/DAIKEN|大量発注/.test(text)) {
      return sentenceJoin([
        "1. DAIKEN関連商品について、通常数量と大口数量で出荷可否が変わるか仕入れ先に確認する。",
        "2. 一定数量以上の注文では自動で問い合わせ導線を出し、即時購入後に欠品連絡となる事故を防ぐ。",
        "3. 大口案件向けに、分納、納期回答、代替メーカー候補を営業側で返せるテンプレートを用意する。"
      ]);
    }
    if (/建売|新築マンション|引渡し|中古リノベ/.test(text)) {
      return sentenceJoin([
    ...
    return sentenceJoin([
      "1. 投稿内の顧客表現を検索キーワードとして登録し、同種投稿を週次で確認する。",
      `2. ${productText} に関する商品ページへ、納期確認、価格改定、代替品、関連副資材の導線を追加する。`,
      "3. SNSで出ている不安をFAQ化し、「いつ届くか」「値上げ前に買えるか」「代替できるか」に答えるコンテンツへ展開する。"
    ]);
  }

  if (/一人親方|小規模事業者|プッシュ型|情報収集|情報発信/.test(text)) {
    return sentenceJoin([
      "1. 一人親方や小規模工務店向けに、長文ニュースではなく「今週注意すべき品目」「発注前に確認すること」「代替候補」の3点に絞った告知枠を作る。",
      "2. 商品ページに表示する注意文を、メーカー発表文ではなく、職人が現場で判断できる短い表現に直す。",
      "3. 仕入れ先確認で得た供給注意情報を、カテゴリページ、メール、営業資料に同時反映できる運用フローを作る。"
    ]);
  }

  if (/ノダ|建具|収納|造作材|階段|框|仕切り価格|7月1日/.test(text)) {
    return sentenceJoin([
      "1. ノダ商品のうち、建具、収納、造作材、階段、框関連の掲載SKUを抽出し、7月1日出荷分からの価格改定対象か確認する。",
      "2. 商品ページと見積回答に、現価格の適用期限、出荷日基準、受注残の扱いを明記する。",
      "3. 値上げ前発注の案内を作る場合は、単体商品だけでなく、金物、接着剤、見切り材など同時購入される副資材もセットで提案する。",
      "4. 永大、DAIKEN、LIXILなど周辺メーカーでも同様の改定がないか、木質建材カテゴリ単位で横展開して確認する。"
    ]);
  }

  if (/チヨダウーテ|ジプスター|石膏ボード|不燃仕様|仕様統合/.test(text)) {
    return sentenceJoin([
      "1. ジプスターの旧仕様、準不燃、不燃仕様、後継品番を商品マスタ上で対応表にする。",
      "2. 商品ページに、仕様統合の開始日、受注開始日、性能区分、既存品番からの置き換え可否を記載する。",
      "3. 見積・問い合わせ対応用に、設計指定が旧仕様の場合の確認文を用意し、勝手な代替提案にならないようにする。",
      "4. 石膏ボード関連の副資材、ビス、目地材、施工部材も一緒に確認し、仕様変更で買い忘れが起きない導線を作る。"
    ]);
  }

  if (/不動産投資|完了検査|引渡し|リフォームオーナー/.test(text)) {
    return sentenceJoin([
      "1. リフォーム・賃貸引き渡し前に必要になりやすい設備まわりの副資材、補修材、内装材を短納期カテゴリとして整理する。",
      "2. 商品一覧で、即納可能、納期確認必要、代替提案可能の表示を分け、急ぎ案件の顧客が迷わない導線にする。",
      "3. リフォーム会社向けに、設備遅延時でも先に進められる工事項目と必要資材をまとめた記事を作成する。",
      "4. 引き渡し遅延に直結する品目は、在庫切れ前にメールで注意喚起し、代替品や分納相談へ誘導する。"
    ]);
  }

  if (/都営住宅|緊急要望/.test(text)) {
    return sentenceJoin([
      `1. 都営住宅工事で挙がっている ${productText} を、きりいーねの商品カテゴリに対応付けたリスク一覧にする。`,
      "2. 仕入れ先へ、対象品目の現在在庫、次回入荷、価格改定予定、代替品可否を確認し、商品ページに反映する。",
      "3. 公的工事で供給不安が出ている品目として、工務店向けに「早期発注推奨リスト」を作成し、メール・営業資料・カテゴリページで案内する。",
      "4. 国交省や東京都の追加発表を監視対象に入れ、需給調査や支援策が出た場合はニュース記事ではなく実務チェックリストとして再編集する。"
    ]);
  }

  if (/ナフサショック|原材料危機|材料そのものが不安定/.test(text)) {
    return sentenceJoin([
      "1. ナフサ由来の影響が出やすい塩ビ、接着剤、断熱材、包装材、樹脂系部材を監視カテゴリとして登録する。",
      "2. 価格改定、受注停止、納期未定、数量制限を別々のステータスとして商品マスタに持たせる。",
      "3. 仕入れ先確認の頻度を上げ、週次で「値上げ」「欠品」「代替可能」「代替不可」に分類した社内共有表を更新する。",
      "4. 顧客向けには不安を煽る表現ではなく、今買える商品、代替候補、納期確認が必要な商品を明確に分けて提示する。"
    ]);
  }

  return sentenceJoin([
    `1. ${productText} に該当する商品・カテゴリを抽出し、価格、納期、在庫、代替可否を確認する。`,
    "2. 記事内のメーカー名・品目名を商品マスタ、FAQ、営業問い合わせテンプレートに反映する。",
    "3. 顧客が判断に迷う点を商品ページに追加し、必要ならカテゴリページやメルマガで注意喚起する。"
  ]);
}
